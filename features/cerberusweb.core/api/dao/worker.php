...
class DAO_Worker extends C4_ORMHelper {
	...
	static function maint() {
		...
		$sql = "DELETE QUICK watcher_filter FROM watcher_filter LEFT JOIN worker ON watcher_filter.worker_id = worker.id WHERE worker.id IS NULL";
		$db->Execute($sql);
		
		$logger->info('[Maint] Purged ' . $db->Affected_Rows() . ' watcher_filter records.');
